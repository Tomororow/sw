	...
	function start(page,threeLevelPermission){
		...
		$("#table_list").empty();
			$.ajax({
								...
								var currentTag = document.getElementById("table_list");
									var newTrTag = document.createElement("tr");
										...
										var newTdTag2 = document.createElement("td");
											newTdTag2.innerHTML=msg[i].userCode;
										var newTdTag3 = document.createElement("td");
											newTdTag3.innerHTML=msg[i].userName;
										var newTdTag4 = document.createElement("td");
											newTdTag4.innerHTML=msg[i].userPwd;
										var newTdTag5 = document.createElement("td");
											newTdTag5.innerHTML=msg[i].realName;
										var newTdTag6 = document.createElement("td");
											newTdTag6.innerHTML=msg[i].mobile;
										var newTdTag7 = document.createElement("td");
												var sonid = JSON.stringify(msg[i].id);
												var innerHTML = showThreeLevelPermission([],sonid);
												newTdTag7.innerHTML=innerHTML;
			 ...
			 $.post(ctxPath+"/user/page_nav").success(function (response) {
	...
	//添加渠道弹窗
	 function addUser(){
		 layer.open({
		        ...
		        ,yes: function(){
		        	...
		        	layer.confirm('添加成功', {
	        			...
	        			document.forms[0].action=ctxPath+"/user/list?state=74";
	  ...
	  function insertSubmit(){
		   var frameId=document.getElementById("insert-user-form").getElementsByTagName("iframe")[0].id;
	...
	//渠道编辑提交===================================
		function doEdit(id){
			layer.open({
		        ...
		        ,shade: 0.8
		        ,maxmin: true
		        ,content: ctxPath+"/user/edit_page?id="+id
		        ,btn: ['确定', '关闭'] //只是为了演示
		        ,yes: function(){
	        	//子页面提交
	        	editSubmit();
