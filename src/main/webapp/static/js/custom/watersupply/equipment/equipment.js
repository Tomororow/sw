...
function start(id,page,threeLevelPermission,level,name){
	debugger
	Addid = id;
	Addname = name;
		//alert(id);
		...
		$.post(ctxPath+"/equipment/page_nav").success(function (response) {
	...
	function add(){
		...
		delCookie("canalId2");
		if(Addid==null||Addid==""){
			layer.alert('请先选择添加设备的站位！', {icon: 0});
		}else{
			//打开添加窗口
			//alert("111");
			 layer.open({
			        ...
			        ,shade: 0.8
			        ,maxmin: true
			        ,content: ctxPath+"/equipment/add_page?id="+Addid
			        ,btn: ['确定', '关闭'] //只是为了演示
			        ,yes: function(){
			        	//子页面提交
			        	addEquipmentSubmit();
	...
	function addEquipmentSubmit(){
	   var frameId=document.getElementById("add-equipment-form").getElementsByTagName("iframe")[0].id;
	   $('#'+frameId)[0].contentWindow.mySubmit();
	   
	   layer.confirm('添加成功', {
 		  btn: ['确定'] //按钮
 		}, function(){	
			document.forms[0].action=ctxPath+"/equipment/list?state=46";
	...
	function doEdit(sluiceId){
		layer.open({
	        ...
	        ,shade: 0.8
	        ,maxmin: true
	        ,content: ctxPath+"/equipment/edit_page?sluiceId="+sluiceId
	        ,btn: ['确定', '关闭'] //只是为了演示
	        ,yes: function(){
        	//子页面提交
        		...
        		layer.confirm('你的预测编辑成功', {
      			...
      			document.forms[0].action=ctxPath+"/equipment/list?state=23";
		    		document.forms[0].submit();
	        	});
	        }
	        ,btn2: function(){
