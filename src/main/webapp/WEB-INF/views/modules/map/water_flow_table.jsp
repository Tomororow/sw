...
<html>
  <head>
    ...
    <title>My JSP 'water_flow_table.jsp' starting page</title>
...
<script src="${ctxstc}/js/custom/water_area_cook.js"></script>
  ...
  <body>
    ...
    <div id="main" style="border-right:1px solid #ccc;margin-left:0px;margin-top:-30px;height: 280px;width:100%"></div>
	<script>
			var a=0;
...
//			var twoSecond = 2 * 1000;
			...
			option = {
				...
				tooltip: {//这部分与提示框有关，与Y坐标自适应无关
					...
					formatter: function (params){//需要接收一个参数数组
						...
						return date.getHours()-1 + '时'+date.getMinutes()+ '分'+  date.getSeconds()+'秒       水面液位:' + params.value[1]+'米';
					},
					axisPointer: {
						animation: true
					}
			...
			var devicewaterCode=getCookie("devicewaterCode");
				if(devicewaterCode==null||devicewaterCode==""){
					return;
				}
				//var time = new Date();
				...
				var current;
				$.ajax({
					url:"${ctx}/site_water_dynamic_info/current_flow",
					data: {devicewaterCode:devicewaterCode}, 	
					type: "post",
					async: false,//非异步
					success: function(msg){
						//alert(JSON.stringify(msg) );
					   current=parseFloat(msg);
