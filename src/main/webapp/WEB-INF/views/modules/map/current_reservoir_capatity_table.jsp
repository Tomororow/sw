...
//			var twoSecond = 2 * 1000;
			...
			option = {
				...
				tooltip: {//这部分与提示框有关，与Y坐标自适应无关
					...
					formatter: function (params){//需要接收一个参数数组
						...
						return date.getHours()-1 + '时'+date.getMinutes()+ '分'+  date.getSeconds()+'秒       库容:' + params.value[1]+'万米3';
					},
					axisPointer: {
						animation: true
					}
			...
			var siteWaterId=getCookie("siteWaterId");
			var deviceReservoirCode=getCookie("deviceReservoirCode")
				//var time = new Date();
				//alert(Date.parse(time));
				//alert(1111);
				if(siteWaterId!=null){//点击图标
					if(lastSiteWaterId==""){
						//alert(1);
						$.ajax({
							url:"${ctx}/site_water_dynamic_info/cycle_data_reservoir_capatity",//拿到前360秒的数据
							data: {siteWaterId:siteWaterId,deviceReservoirCode:deviceReservoirCode,time:120}, 	
							type: "post",
							async: false,//非异步
						...
						if(siteWaterId==lastSiteWaterId){
				...
				var current;
				$.ajax({
					url:"${ctx}/site_water_dynamic_info/current_reservoir_capatity",
					data: {siteWaterId:siteWaterId,deviceReservoirCode:deviceReservoirCode}, 	
					type: "post",
					async: false,//非异步
					success: function(msg){
					   current=msg.totalCapacity;
					   //alert(current);
