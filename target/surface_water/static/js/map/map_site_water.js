...
//水位站
function start(id){
	//setCookie("siteEntitys",siteEntitys);
	...
	$.ajax({
		...
		success: function(msg){
			for(var i=0;i<msg.length;i++){
					siteEntitys.add({
					  id:msg[i].id,//msg[i].swiLongitude, msg[i].swiLatitude
					  position : Cesium.Cartesian3.fromDegrees(msg[i].swiLongitude, msg[i].swiLatitude,msg[i].swiHeight),
					  name:'0',
					  billboard : {
					    image : 'http://localhost:8080/surface_water/static/3D/shuiwei.png',
					    width : 45,
					    height : 50
					  },
					  label : {
					    text :msg[i].deviceWaterName,
