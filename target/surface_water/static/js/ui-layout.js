						...
						function($parse) {
							...
							return {
								...
								compile : function compile(tElement, tAttrs) {
									...
									var opts = angular.extend({}, $parse(
											tAttrs.uiLayout)(), $parse(
											tAttrs.options)());
									...
									for (_i = 0; _i < _child_len; ++_i) {
										angular.element(_childens[_i])
												.addClass('stretch');
									}
									if (_child_len > 1) {
										var flowProperty = isUsingColumnFlow ? 'left'
												: 'top';
										var oppositeFlowProperty = isUsingColumnFlow ? 'right'
												: 'bottom';
										...
										for (_i = 0; _i < _child_len; ++_i) {
											if (_i == 0) {
												/*
												 * var area =
												 * angular.element(_childens[_i]).css(flowProperty,
												 * step * _i +
												 * '%').css(oppositeFlowProperty,
												 * 100 - step * (_i + 1) + '%');
												 * if (_i < _child_len - 1) {
												 * var bar =
												 * angular.element(splitBarElem_htmlTemplate).css(flowProperty,
												 * step * (_i + 1) + '%');
												 * area.after(bar); }
												 */
												var area = angular
														.element(_childens[_i])
														.css(flowProperty, '0%')
														.css(oppositeFlowProperty,'87%');
												if (_i < _child_len - 1) {
													var bar = angular
															.element(splitBarElem_htmlTemplate)
															.css(flowProperty,'13%');
													area.after(bar);
												}
											} else {
												var area = angular
														.element(_childens[_i])
														.css(flowProperty,'13%')
														.css(oppositeFlowProperty,'0%');
												if (_i < _child_len - 1) {
													var bar = angular
															.element(splitBarElem_htmlTemplate)
															.css(flowProperty,'100%');
													area.after(bar);
												}
											}
										}
				...
				function() {
					...
					return {
						...
						link : function(scope, iElement, iAttrs, parentLayout) {
							...
							var mouseProperty = isUsingColumnFlow ? 'clientX'
									: 'clientY';
							var flowProperty = isUsingColumnFlow ? 'left'
									: 'top';
							var oppositeFlowProperty = isUsingColumnFlow ? 'right'
									: 'bottom';
							var sizeProperty = isUsingColumnFlow ? 'width'
									: 'height';
							var barElm = iElement[0];
							function _cached_layout_values() {
								var layout_bb = parentLayout.element[0]
							...
							function _draw() {
								var the_pos = (lastX - _cache.layoutOrigine)
										/ _cache.layoutSize * 100;
								the_pos = Math.min(the_pos, 100
										- _cache.barSize / _cache.layoutSize
										* 100);
								the_pos = Math
										.max(
												the_pos,
												parseInt(
														barElm.previousElementSibling.style[flowProperty],
														10));
								...
								barElm.style[flowProperty] = barElm.nextElementSibling.style[flowProperty] = the_pos
										+ '%';
								barElm.previousElementSibling.style[oppositeFlowProperty] = 100
										- the_pos + '%';
							...
							function _resize(mouseEvent) {
								...
								if (!_cache.time
										|| +new Date() > _cache.time + 1000) {
									_cached_layout_values();
								}
								animationFrameRequested = window
