...
function init() {
	...
	//编辑页面初始标注
	var devicelng = $("#longitude").val();
	var devicelat = $("#latitude").val();
	if(devicelng!=null && devicelng!="" && devicelat!=null && devicelat!=""){
		...
		$("#localAddress").val($("#installAddress").val());
		$("#searchArea #keyword").val($("#installAddress").val());
	}else{
...
function returnValue(obj){
	...
	if(isNotEmpty(localLng) && isNotEmpty(localLat) && isNotEmpty(localAddress)){
		...
		$("#longitude").val(localLng);
		$("#latitude").val(localLat);
		$('#installAddress').val(localAddress);
