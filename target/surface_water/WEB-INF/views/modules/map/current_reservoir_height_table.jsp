			...
			option = {
				...
				tooltip: {//这部分与提示框有关，与Y坐标自适应无关
					...
					formatter: function (params){//需要接收一个参数数组
						...
						return date.getHours()-1 + '时'+date.getMinutes()+ '分'+  date.getSeconds()+'秒      水库水位:' + params.value[1]+'米';
					},
					axisPointer: {
						animation: true
					}
			...
			var siteWaterId=getCookie("siteWaterId");
				...
				if(siteWaterId!=null){//点击图标
					if(lastSiteWaterId==""){
						//alert(1);
						$.ajax({
							url:"${ctx}/site_water_dynamic_info/cycle_data_reservoir",//拿到前360秒的数据
							data: {siteWaterId:siteWaterId,time:120}, 	
							type: "post",
							async: false,//非异步
				...
				var current;
				$.ajax({
					url:"${ctx}/site_water_dynamic_info/current_reservoir",
					data: {siteWaterId:siteWaterId}, 	
					type: "post",
					async: false,//非异步
					success: function(msg){
